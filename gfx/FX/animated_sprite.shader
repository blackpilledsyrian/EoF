...
PixelShader =
{
	...
	[[
		...
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			int nb_frames = int(frame_size.x * 256) + 1;
			...
			int offsetInt = int(Offset.x*nb_frames);
			int start_frame   = 0x3F;
            int end_frame     = 0x3F + 1;
            int frame_per_sec = 0xFF;
            int looping       = 0x1;
			...
			if ((!bool(looping)) && (delta_time >= 1.0) ) {
                current_frame_offset = end_frame - start_frame - 1;
            }

            v.vTexCoord.x += (start_frame + current_frame_offset)/nb_frames;
